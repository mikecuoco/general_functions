...
# This code will be executed for all other lines:
{
    ...
    if (count) {
        # If this is the second time we've seen this read ID, we should print the line
        # that we got when we first saw the read ID
        if (count == 1) {
            print a;
        }
        # Send the entirety of the current line to stdout
        print $0;
    } else {
        ...
        a = $0;
        # Note that we don't print anything else here!
    }
}
